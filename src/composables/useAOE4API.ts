...
export default function useAOE4API() {
	async function fetchPlayerRecentGames(playerId: string): Promise<GameResult[]> {
		const url: string = `https://aoe4world.com/api/v0/players/${playerId}/games?limit=20`
		...
		const games: GameResult[] = data.games.slice(0, 20).map((game: any) => {
			...
			if (ourTeamIndex !== -1) {
				...
				ourTeam.forEach((playerWrapper: any) => {
					const elo: number | null =
						playerWrapper.player.mmr || playerWrapper.player.rating
				...
				if (elos.length > 0) {
					ownTeamElo =
						elos.reduce((sum: number, elo: number) => sum + elo, 0) / elos.length
				}
			}
			...
			if (ourTeamIndex !== -1) {
				...
				if (allOpponentElos.length > 0) {
					opponentTeamElo =
						allOpponentElos.reduce((sum: number, elo: number) => sum + elo, 0) /
						allOpponentElos.length
				}
			}
